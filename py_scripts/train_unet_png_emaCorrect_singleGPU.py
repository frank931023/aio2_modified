...
def main():
    ...
    assert ndn_split[1]=='seg', "Directory name of noisy labels is invalid!"
    ...
    for epoch in range(last_epoch, fepochs):
        ...
        for bi, batch in enumerate(train_loader):
            ...
            if correct:
                ...
                with torch.no_grad():
                    if args.cbase == 'iter':
                        if args.cmd == 'teacher':
                            pred_logits_it = net_ema_it(images).squeeze(axis=1) 
                        elif args.cmd == 'student':
                            pred_logits_it = net(images).squeeze(axis=1) 
                        else:
                            raise NameError("Correction model type is invalid!")
                    elif args.cbase == 'epoch':
                        # net_ema_it_cp is updated using net if cmd==student, otherwise using net_ema_it
                        # so already align with cmd
                ...
                if args.cmeth == 'object':
                    ...
                    true_masks = use.pixel_wise_label_correction(ns_masks, pred_logits_it, 
                                                                 confidence=args.pconf)
                      
                # d - accs of corrected labels
                if args.sc_sfs<=0:
                    ns_accs, ns_dict = evl.evaluate_batch(true_masks, gt_masks, net.n_classes, 
        ...
        # save checkpoints
        # if ((epoch+1)%args.save_inv==0 and not correct) or (epoch-n_back >= args.epochs):
